...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	static final int RELEASE_TO_REFRESH_Head = 0x1;
	static final int REFRESHING = 0x2;
	static final int MANUAL_REFRESHING = 0x3;
	static final int RELEASE_TO_REFRESH_Foot = 0x4;
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP: {
			if (isBeingDragged) {
				...
				System.out.println(">>>>>>>>>>>  ACTION_UP  state " + state);
				if (state == RELEASE_TO_REFRESH_Head && null != onHeaderRefreshListener) {
					setRefreshingInternal(true);
					onHeaderRefreshListener.onRefresh();

				} else if (state == RELEASE_TO_REFRESH_Foot && null != onFooterRefreshListener) {
					setRefreshingInternal(true);
...
//						}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String pullLabel1 = context.getString(R.string.pull_to_refresh_pull_label1);
		String refreshingLabel1 = context.getString(R.string.pull_to_refresh_refreshing_label1);
		String releaseLabel1 = context.getString(R.string.pull_to_refresh_release_label1);
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH, releaseLabel1, pullLabel1, refreshingLabel1);
	...
	private boolean pullEvent() {
		...
		if (newHeight != 0) {
			if (state == PULL_TO_REFRESH && headerHeight < Math.abs(newHeight)) {
				System.out.println(">>>>>>>>>>>11  pullEvent() currentMode " + currentMode);
				switch (currentMode) {
				case MODE_PULL_UP_TO_REFRESH:
					state = RELEASE_TO_REFRESH_Foot;
					footerLayout.releaseToRefresh();
					// state = 4;
					break;
				case MODE_PULL_DOWN_TO_REFRESH:
					state = RELEASE_TO_REFRESH_Head;
					// state = 1;
			...
			} else if (state == RELEASE_TO_REFRESH_Head && headerHeight >= Math.abs(newHeight)) {
				state = PULL_TO_REFRESH;

				System.out.println(">>>>>>>>>>>  pullEvent() currentMode " + currentMode);
				switch (currentMode) {
				case MODE_PULL_UP_TO_REFRESH:
				...
				return true;
			} else if (state == RELEASE_TO_REFRESH_Foot && headerHeight >= Math.abs(newHeight)) {
				state = PULL_TO_REFRESH;
