...
import com.zmsoft.widget.pull.internal.FlipLoadingLayout;
import com.zmsoft.widget.pull.internal.LoadingLayoutNew;
import com.zmsoft.widget.pull.internal.RotateLoadingLayout;
import com.zmsoft.widget.pull.internal.ViewCompat;

public abstract class PullToRefreshBaseNew<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private LoadingLayoutNew mHeaderLayout;
    private LoadingLayoutNew mFooterLayout;
    ...
    private int mScrollValue; // 获取滑动的实际距离

    private OnMyScrollListener mOnMyScrollListener = null;
    ...
    public PullToRefreshBaseNew(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public PullToRefreshBaseNew(Context context, Mode mode) {
        super(context);
        mMode = mode;
        init(context, null);
    }

    public PullToRefreshBaseNew(Context context, Mode mode, AnimationStyle animStyle) {
        super(context);
        mMode = mode;
    ...
    protected LoadingLayoutNew createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
        LoadingLayoutNew layout = mLoadingAnimationStyle.createLoadingLayout(context, mode,
    ...
    protected final LoadingLayoutNew getFooterLayout() {
    ...
    protected final LoadingLayoutNew getHeaderLayout() {
        ...
        switch (getPullToRefreshScrollDirection()) {
            case VERTICAL:
                ...
                if (mOnMyScrollListener != null) {
                    mOnMyScrollListener.move(value);
                }
                break;
            case HORIZONTAL:
                scrollTo(value, 0);
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // Styleables from XML
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullToRefreshNew);

        if (a.hasValue(R.styleable.PullToRefreshNew_ptrMode)) {
            mMode = Mode.mapIntToValue(a.getInteger(R.styleable.PullToRefreshNew_ptrMode, 0));
        }

        if (a.hasValue(R.styleable.PullToRefreshNew_ptrAnimationStyle)) {
            mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
                    R.styleable.PullToRefreshNew_ptrAnimationStyle, 0));
        }
         ...
         * Styleables from XML
         */
        if (a.hasValue(R.styleable.PullToRefreshNew_ptrRefreshableViewBackground)) {
            Drawable background = a.getDrawable(R.styleable.PullToRefreshNew_ptrRefreshableViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        } else if (a.hasValue(R.styleable.PullToRefreshNew_ptrAdapterViewBackground)) {
//            Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
            Drawable background = a.getDrawable(R.styleable.PullToRefreshNew_ptrAdapterViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        }

        if (a.hasValue(R.styleable.PullToRefreshNew_ptrOverScroll)) {
            mOverScrollEnabled = a.getBoolean(R.styleable.PullToRefreshNew_ptrOverScroll, true);
        }
        ...
        if (a.hasValue(R.styleable.PullToRefreshNew_ptrScrollingWhileRefreshingEnabled)) {
            mScrollingWhileRefreshingEnabled = a.getBoolean(
                    R.styleable.PullToRefreshNew_ptrScrollingWhileRefreshingEnabled, false);
        }
    ...
    public static enum AnimationStyle {
        ...
        LoadingLayoutNew createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
            switch (this) {
                case ROTATE:
                default:
    ...
    public static interface OnPullEventListener<V extends View> {
         ...
         */
        public void onPullEvent(final PullToRefreshBaseNew<V> refreshView, State state, Mode direction);
    ...
    public static interface OnRefreshListener<V extends View> {
         ...
         * end
         */
        public void onRefresh(final PullToRefreshBaseNew<V> refreshView);
    ...
    public static interface OnRefreshListener2<V extends View> {
         ...
         */
        public void onPullDownToRefresh(final PullToRefreshBaseNew<V> refreshView);

        /**
         * onPullUpToRefresh will be called only when the user has Pulled from
         * the end, and released.
         */
        public void onPullUpToRefresh(final PullToRefreshBaseNew<V> refreshView);
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mContinueRunning && mScrollToY != mCurrentY) {
                ViewCompat.postOnAnimation(PullToRefreshBaseNew.this, this);
            } else {
                if (null != mListener) {
    ...
    public OnMyScrollListener getOnMyScrollListener() {
        return mOnMyScrollListener;
    }

    public void setOnMyScrollListener(OnMyScrollListener mOnScrollListener) {
        this.mOnMyScrollListener = mOnScrollListener;
    }

    public interface OnMyScrollListener {
        void move(int scrollValue);
    }
}
